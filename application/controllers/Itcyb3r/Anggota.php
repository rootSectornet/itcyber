...
class Anggota extends CI_Controller {
	...
	// start of profile
		public function profile($id){
			$data['artikel'] = $this->Artikel_model->get_byAuthor($id);
			$data['anggota'] = $this->Anggota_model->get_byID($id);
			$data['comment'] = $this->Comment_model->Comment_BdBaca();
			$data['comment_all'] = $this->Comment_model->Comment_all();
			$data['galery'] = $this->Galery_model->get_BYid($id);
			$this->load->view('backend/template/header');
			$this->load->view('backend/template/menu',$data);
			$this->load->view('backend/anggota/anggota/profile',$data);
			$this->load->view('backend/template/footer');
		}
	// end of profile
	// start of update profile
		public function update_profil(){
	        ...
	        $condition['id_anggota'] = $this->input->post('id');
	        $this->Anggota_model->update($condition,$data);
			$this->session->set_flashdata('anggota','
	              <div class="alert alert-success alert-dismissible">
	                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
	        ...
	        redirect('itcyb3r/anggota/profile/'.$this->input->post('id'));
		}
